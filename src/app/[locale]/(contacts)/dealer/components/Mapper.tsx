...
const Mapp = ({ name, image }: { name?: string; image: string }) => {
  ...
  return (
    <div className="flex h-[calc(100vh-220px)] mb-[10-vh]">
      {/* SIDEBAR */}
      <div
        className="w-[400px] min-w-[300px] max-w-[500px] overflow-y-auto bg-white p-8 shadow-md"
        onWheel={(e) => e.stopPropagation()}
      >
        <p className="text-2xl font-bold mb-6">{t('find_dealer')}</p>
        ...
        >
          {markers.map((loc, index) => (
            <Accordion.Item key={index} value={loc.name}>
              ...
              <Accordion.Panel className="bg-slate-200 pt-2 space-y-1">
